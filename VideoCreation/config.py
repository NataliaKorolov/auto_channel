...
class VideoConfig:
    """Base configuration management for video creation workflows"""
    ...
    def get_required_directories(self) -> list:
        """Get list of all required directories for validation - to be overridden by subclasses"""
...
class VideoConfigGreece(VideoConfig):
    """Greece-specific video configuration management"""
    
    def __init__(self, current_project_dir: str, base_path: str = None, environment: str = "local"):
        """
        Initialize Greece configuration
        
        Args:
            current_project_dir: Current Greece project directory name (e.g., "3_Hector")
            base_path: Base directory path (auto-detected if None)
            environment: "local" or "colab"
        """
        super().__init__(base_path, environment)
        self.current_project_dir = current_project_dir
        self._setup_greece_paths()
    
    def _setup_greece_paths(self):
        """Setup all Greece workflow paths based on current project directory"""
        # Base Greece directories
        self.BASE_DIRECTORY_GREECE = os.path.join(self.BASE_DIRECTORY, "Greece_Automation")
        self.BASE_DIRECTORY_GREECE_CURRENT = os.path.join(self.BASE_DIRECTORY_GREECE, self.current_project_dir)
        ...
        self.intro_paths = {
            ...
            "video_paths": [os.path.join(self.INTRO_BASE, f"Intro_{i}.mp4") for i in range(1, 2)],
        ...
        self.tail_paths = {
            ...
            "video_paths": [os.path.join(self.TAIL_BASE, f"Tail_{i}.mp4") for i in range(1, 3)]
        }
        ...
        self.current_paths = {
            "video_paths": [os.path.join(self.current_artifacts, f"Prompt {i}.mp4") for i in range(1, 11)],
    ...
    def change_current_project_directory(self, new_project_dir: str):
        """
        Change the current project directory and update all related paths
        
        Args:
            new_project_dir: New project directory name (e.g., "4_Athena")
        """
        print(f"🔄 Changing project directory from '{self.current_project_dir}' to '{new_project_dir}'")
        self.current_project_dir = new_project_dir
        self._setup_greece_paths()
        print(f"✅ Project directory updated successfully")
    ...
    def get_video_paths_for_workflow(self, workflow_part: str) -> List[str]:
        ...
        if workflow_part == "intro":
            return list(self.intro_paths["video_paths"])
        elif workflow_part == "tail":
            return list(self.tail_paths["video_paths"])
        elif workflow_part == "current":
            return list(self.current_paths["video_paths"])
    ...
    def get_required_directories(self) -> list:
        """Get list of all required directories for Greece workflow validation"""
        return [
...
# TODO: Add VideoConfigTT class for TimelessTales workflow when needed
class VideoConfigTT(VideoConfig):
    """TimelessTales-specific video configuration management"""
    
    def __init__(self, base_path: str = None, environment: str = "local"):
        """Initialize TT configuration - implementation to be added later"""
        super().__init__(base_path, environment)
        self._setup_tt_paths()
    
    def _setup_tt_paths(self):
        """Setup TimelessTales paths - placeholder for future implementation"""
        self.TT_DIRECTORY = os.path.join(self.BASE_DIRECTORY, "TT")
        # More paths to be added when implementing TT workflow
    
    def get_required_directories(self) -> list:
        """Get list of all required directories for TT workflow validation"""
        return [self.TT_DIRECTORY]


# Factory functions for easy initialization
def get_local_config(current_project_dir: str) -> VideoConfigGreece:
    """
    Get Greece configuration for local development
    
    Args:
        current_project_dir: Current Greece project directory (e.g., "3_Hector")
    """
    return VideoConfigGreece(current_project_dir=current_project_dir, environment="local")

def get_colab_config(current_project_dir: str, base_path: str = None) -> VideoConfigGreece:
    """
    Get Greece configuration for Google Colab
    
    Args:
        current_project_dir: Current Greece project directory (e.g., "3_Hector")
        base_path: Base directory path (auto-detected if None)
    """
    return VideoConfigGreece(current_project_dir=current_project_dir, base_path=base_path, environment="colab")